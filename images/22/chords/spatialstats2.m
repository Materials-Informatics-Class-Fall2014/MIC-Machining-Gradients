function spatialstats2()

close all;
N = 1499; % use bottom N pixels
Np = 50; % plot bottom Np pixels    Np <= N
...
try
     P1 = PC(1:mm*size(cordsN,3),:);
end
try
     P2 = PC(mm*size(cordsN,3)+1:2*mm*size(cordsN,3),:);
end
try
     P3 = PC(2*mm*size(cordsN,3)+1:3*mm*size(cordsN,3),:);
end
try
     P4 = PC(3*mm*size(cordsN,3)+1:4*mm*size(cordsN,3),:);
end
try
     P5 = PC(4*mm*size(cordsN,3)+1:5*mm*size(cordsN,3),:);
end
try
     P6 = PC(5*mm*size(cordsN,3)+1:6*mm*size(cordsN,3),:);
end
try
     P7 = PC(6*mm*size(cordsN,3)+1:7*mm*size(cordsN,3),:);
end
...
%     factor = norm(X-approx);
 ...
 if avg == 1
    ...
    top = [1:Np,size(EDG,1)+1:size(EDG,1)+Np,2*size(EDG,1)+1:2*size(EDG,1)+Np,3*size(EDG,1)+1:3*size(EDG,1)+Np,4*size(EDG,1)+1:4*size(EDG,1)+Np];
    bot = [size(EDG,1)-Np+1:size(EDG,1),2*size(EDG,1)-Np+1:2*size(EDG,1),3*size(EDG,1)-Np+1:3*size(EDG,1),4*size(EDG,1)-Np+1:4*size(EDG,1),5*size(EDG,1)-Np+1:5*size(EDG,1)];
    zz =  [1:size(EDG,1),1:size(EDG,1),1:size(EDG,1),1:size(EDG,1),1:size(EDG,1)];
 end
     
     
try
     plot3(P1(:,1),P1(:,2),zz,'ro','MarkerFaceColor','r','markeredgecolor','k','MarkerSize',10); hold on; 
     i =1;
end
try
     plot3(P2(:,1),P2(:,2),zz,'go','MarkerFaceColor','g','markeredgecolor','k','MarkerSize',10);
     i = 2;
end
try
     plot3(P3(:,1),P3(:,2),zz,'bo','MarkerFaceColor','b','markeredgecolor','k','MarkerSize',10);
     i = 3;
end
try
     plot3(P4(:,1),P4(:,2),zz,'ko','MarkerFaceColor','k','markeredgecolor','k','MarkerSize',10);
     i = 4;
end
try
     plot3(P5(:,1),P5(:,2),zz,'co','MarkerFaceColor','c','markeredgecolor','k','MarkerSize',10);
     i = 5;
end
try
     plot3(P6(:,1),P6(:,2),zz,'mo','MarkerFaceColor','m','markeredgecolor','k','MarkerSize',10);
     i = 6;
     
end
try
     plot3(P7(:,1),P7(:,2),zz,'yo','MarkerFaceColor','y','markeredgecolor','k','MarkerSize',10);
     i = 7;
end
